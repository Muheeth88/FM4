...
@router.post("/{project_id}/invoke-planner")
async def invoke_planner(project_id: str):
    """Invoke the planner agent with context generated by context builder."""
    project = ProjectService.get_project(project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    try:
        workflow_service = MigrationWorkflowService()
        return workflow_service.invoke_planner(project_id)
    except FileNotFoundError as e:
        logger.error(f"Context preparation failed for {project_id}: {e}")
        raise HTTPException(status_code=400, detail=str(e))
    except ValueError as e:
        logger.error(f"Planner execution failed for {project_id}: {e}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
